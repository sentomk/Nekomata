...
[[noreturn]] void reject(wasm_offer_error_code code, std::string_view source, std::size_t line,
  ...
  throw wasm_offer_error(code, line,
                         "invalid wasm offer '" + std::string(source) + "' at line " +
...
void validate_wasm_offer(const wasm_offer& offer, std::string_view source) {
  ...
  require_portable_path(offer.artifact_path, "artifact", true, source, 0);
  ...
  for (const auto& entry : offer.entries) {
    require_portable_path(entry, "entry", false, source, 0, wasm_offer_error_code::invalid_field);
...
wasm_offer parse_wasm_offer(std::string_view text, std::string_view source) {
  ...
  const auto magic_issue =
      inspect_magic_line(line, wasm_offer_format_name, wasm_offer_format_version);
  if (magic_issue != magic_line_issue::none) {
    ...
    reject(code, source, line_number,
           "expected " + compose_magic_line(wasm_offer_format_name, wasm_offer_format_version));
  }
  ...
  while (std::getline(input, line)) {
    ...
    if (directive == "artifact") {
      ...
      offer.artifact_path = read_quoted(row, source, line_number, "artifact");
      offer.sha256 = read_quoted(row, source, line_number, "sha256");
      require_end(row, source, line_number);
      require_portable_path(offer.artifact_path, "artifact", true, source, line_number);
    ...
    if (directive == "entry") {
      ...
      require_end(row, source, line_number);
      require_portable_path(entry, "entry", false, source, line_number);
      if (!entries.insert(entry).second) {
...
std::string serialize_wasm_offer(const wasm_offer& offer) {
  ...
  output << compose_magic_line(wasm_offer_format_name, wasm_offer_format_version) << '\n';
...
wasm_offer_ordering compare_wasm_offers(const wasm_offer& incoming,
                                        const wasm_offer* last) noexcept {
  if (last == nullptr || incoming.sequence > last->sequence) {
    return wasm_offer_ordering::supersede;
  }
  if (incoming.sequence < last->sequence) {
    return wasm_offer_ordering::stale;
  }
  if (incoming == *last) {
    return wasm_offer_ordering::duplicate;
  }
  return wasm_offer_ordering::conflict;
}
