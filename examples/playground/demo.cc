// demo.cc —— 可热改的翻译单元（阶段一 playground）
//
...
void step_world() {
    ...
    if (g_x > 76.0) g_vx = -fabs(g_vx);  // 右墙
    if (g_x < 1.0) g_vx = fabs(g_vx);    // 左墙
    ++g_frame;
}
...
void render_world() {
    ...
    std::printf("\033[23;1H[%s] frame=%d  pos=(%.1f, %.1f)  vel=(%.1f, %.1f)\033[K",
                kPhysName, g_frame, g_x, g_y, g_vx, g_vy);
    std::fflush(stdout);
}
