...
#                  EXPORT_HEADER and EXPORT_NAMESPACE let the adapter generate
#                  the side-module descriptor from declared application functions.
...
function(nekomata_add_reload_group name)
  cmake_parse_arguments(ARG ""
    "GROUP_ID;ABI_ID;OFFER_ROOT;MANIFEST_URL;PLT_HEADER;EXPORT_HEADER;EXPORT_NAMESPACE"
    "SOURCES;UNITS;ENTRIES" ${ARGN})
  if(ARG_UNPARSED_ARGUMENTS)
  ...
  if(CMAKE_SYSTEM_NAME STREQUAL "Emscripten")
    ...
    endforeach()
    # PLT_HEADER names an application header defining one typed
    # neko::wasm::plt_slot plus a forwarding trampoline per entry. Compiling
    # it into this generated TU places the trampolines in the main module, so
...
")
    ...
    if(ARG_EXPORT_NAMESPACE AND NOT ARG_EXPORT_HEADER)
      message(FATAL_ERROR "nekomata_add_reload_group(${name}): EXPORT_NAMESPACE requires EXPORT_HEADER")
    endif()
    if(ARG_EXPORT_HEADER)
      cmake_path(ABSOLUTE_PATH ARG_EXPORT_HEADER NORMALIZE
        BASE_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}" OUTPUT_VARIABLE export_header)
      if(NOT EXISTS "${export_header}")
        message(FATAL_ERROR "nekomata_add_reload_group(${name}): EXPORT_HEADER "
          "\"${ARG_EXPORT_HEADER}\" does not exist")
      endif()
      if(export_header MATCHES "[\\\\\"\r\n]")
        message(FATAL_ERROR "nekomata_add_reload_group(${name}): EXPORT_HEADER path "
          "must not contain backslashes, quotes, or newlines")
      endif()
      if(ARG_EXPORT_NAMESPACE AND NOT ARG_EXPORT_NAMESPACE MATCHES
          "^[A-Za-z_][A-Za-z_0-9]*(::[A-Za-z_][A-Za-z_0-9]*)*$")
        message(FATAL_ERROR "nekomata_add_reload_group(${name}): EXPORT_NAMESPACE "
          "must be a C++ namespace")
      endif()
      list(LENGTH ARG_ENTRIES export_entry_count)
      set(cpp_export_entries "")
      foreach(entry IN LISTS ARG_ENTRIES)
        if(NOT entry MATCHES "^[A-Za-z_][A-Za-z_0-9]*$")
          message(FATAL_ERROR "nekomata_add_reload_group(${name}): generated descriptor "
            "requires C++ identifier entry names")
        endif()
        _nekomata_cpp_string(cpp_entry "${entry}")
        if(ARG_EXPORT_NAMESPACE)
          set(cpp_symbol "${ARG_EXPORT_NAMESPACE}::${entry}")
        else()
          set(cpp_symbol "${entry}")
        endif()
        string(APPEND cpp_export_entries
          "{${cpp_entry}, reinterpret_cast<neko::wasm::module_function>(&${cpp_symbol})},\n")
      endforeach()
      set(descriptor_tu "${wasm_dir}/descriptor.cpp")
      file(GENERATE OUTPUT "${descriptor_tu}" CONTENT
"// Generated by the build adapter; application code only defines functions.
#include <neko/detail/wasm_module_descriptor.hpp>
#include \"${export_header}\"
namespace {
const neko::wasm::module_entry entries[] = { ${cpp_export_entries} };
const neko::wasm::module_descriptor descriptor{
    {neko::wasm::module_interface_version, sizeof(neko::wasm::module_descriptor)},
    ${cpp_abi}, ${export_entry_count}, entries};
}
extern \"C\" __attribute__((visibility(\"default\")))
const neko::wasm::module_header* neko_wasm_descriptor() {
  return &descriptor.header;
}
")
      add_library(${name}_neko_descriptor OBJECT "${descriptor_tu}")
      target_link_libraries(${name}_neko_descriptor PRIVATE nekomata::neko)
      target_compile_options(${name}_neko_descriptor PRIVATE ${wasm_compile_flags})
      set_property(TARGET ${name}_neko_descriptor PROPERTY POSITION_INDEPENDENT_CODE ON)
      list(APPEND wasm_objects "$<TARGET_OBJECTS:${name}_neko_descriptor>")
      list(APPEND wasm_depends "${name}_neko_descriptor")
    endif()
  ...
  if(ARG_EXPORT_HEADER OR ARG_EXPORT_NAMESPACE)
    message(FATAL_ERROR "nekomata_add_reload_group(${name}): EXPORT_HEADER and "
      "EXPORT_NAMESPACE apply to Emscripten groups only")
  endif()
