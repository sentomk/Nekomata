...
process_symbols::process_symbols() {
    ...
    const std::vector<Elf64_Shdr> shdrs([&] {
        ...
        for (std::uint16_t i = 0; i < shnum; ++i) {
            out[i] = *reinterpret_cast<const Elf64_Shdr*>(
                bounds(bytes, ehdr->e_shoff + i * ehdr->e_shentsize, sizeof(Elf64_Shdr),
                       "section header"));
        }
        return out;
    }());
    ...
    std::fprintf(stderr, "[neko] process symbols: %zu functions, %zu globals\n", functions_.size(),
                 globals_.size());
}
