...
TEST_CASE("reservations land within rel32 reach and can be committed") {
  ...
  auto allocation = pages.reserve_code_near(kHint, 16);
...
TEST_CASE("committing into an unknown pointer is rejected") {
  ...
  auto foreign = other_pages.reserve_code_near(kHint, 1);
...
TEST_CASE("destroying a candidate allocation unmaps its reservation") {
  neko::elf::code_pages pages;
  ...
  unsigned char residency = 0;
  errno = 0;
  CHECK(mincore(address, system_page_size(), &residency) == -1);
  CHECK(errno == ENOMEM);
}
